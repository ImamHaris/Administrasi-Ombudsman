...
class Sibers extends CI_Controller {
    ...
    public function dashboard(){
        ...
        $data = array(
            ...
            'total_surat_klarifikasi_keluar2' => $this->surat_model->get_total_row_surat_klarifikasi_keluar2(),
            'total_surat_klarifikasi_masuk' => $this->surat_model->get_total_row_surat_klarifikasi_masuk(),
    ...
    public function login() {
        ...
        $data['title'] = "Login";
        $this->load->view('sibers/login', $data);
    }

    public function do_login() {
        $username = $this->security->xss_clean($this->input->post('username'));
        $password = md5($this->security->xss_clean($this->input->post('password')));
        $userdata = $this->sibers_model->get_userdata($username, $password);
    ...
    public function edit_instansi() {
        ...
        if ($go_to == "act_edit") {
            if ($this->upload->do_upload('logo')) {
                ...
                $this->sibers_model->update_instansi_logo($nama, $alamat, $kepala, $nip_kepala, $upload_data, $id_post);
            } else {
                $this->sibers_model->update_instansi($nama, $alamat, $kepala, $nip_kepala, $id_post);
            }
            $this->session->set_flashdata('message', message_box('Data telah diperbaharui'));
            redirect('sibers/edit_instansi');
        } else {
            $data['data'] = $this->sibers_model->get_instansi();
            $data['page'] = "sibers/form_instansi";
        }
    ...
    public function manage_user() {
        ...
        /* pagination */
        $total_row = $this->sibers_model->get_user();
        $per_page = 10;
        $awal = $this->uri->segment(4, 0);
        $akhir = $per_page;
        $data['pagi'] = _page($total_row, $per_page, 4, site_url('sibers/manage_user/page'));
        ...
        if ($go_to == "del") {
            $this->sibers_model->delete_user($id_url);
            $this->session->set_flashdata('message', message_box('Data telah dihapus'));
            redirect('sibers/manage_user');
        } else if ($go_to == "cari") {
            $data['data'] = $this->sibers_model->cari_user($cari);
            $data['page'] = "sibers/list_manage_user";
        } else if ($go_to == "add") {
            $data['datpil'] = $this->sibers_model->get_user_with_id($id_url);
            $data['page'] = "sibers/form_manage_user";
        } else if ($go_to == "edit") {
            $data['datpil'] = $this->sibers_model->get_user_with_id($id_url);
            $data['page'] = "sibers/form_manage_user";
        } else if ($go_to == "act_add") {
            $this->sibers_model->insert_user($username, $password, $nama, $nip, $level);
            $this->session->set_flashdata('message', message_box('Data telah ditambah'));
            redirect('sibers/manage_user');
        } else if ($go_to == "act_edit") {
            if ($password == md5("-")) {
                $this->sibers_model->update_user($username, $nama, $nip, $level, $id_post);
            } else {
                $this->sibers_model->update_user_with_password($username, $password, $nama, $nip, $level, $id_post);
            }
            $this->session->set_flashdata('message', message_box('Data telah diperbaharui'));
            redirect('sibers/manage_user');
        } else {
            $data['data'] = $this->sibers_model->get_user_with_limit($awal, $akhir);
            $data['page'] = "sibers/list_manage_user";
        }
