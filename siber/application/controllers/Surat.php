...
class Surat extends CI_Controller {
    ...
    public function surat_keluar_laporan() {
        ...
        if ($mau_ke == "del") {
            ...
            $data['data'] = $this->surat_model->select_surat_keluar_laporan_limit($awal, $akhir);
            $data['page'] = "surat/list_surat_keluar_laporan";
        }
        $data['title'] = "Surat keluar";
        $this->load->view('sibers/header', $data);
    ...
    public function surat_klarifikasi_keluar() {
        ...
        /* pagination */
        $total_row = $this->surat_model->get_total_row_surat_klarifikasi_keluar();
        $per_page = 10;
        $awal = $this->uri->segment(4, 0);
        $akhir = $per_page;
        $data['pagi'] = _page($total_row, $per_page, 4, site_url('surat/surat_klarifikasi_keluar/page'));
        ...
        //upload config 
        $config['upload_path'] = './assets/upload/surat_klarifikasi_keluar';
        $config['allowed_types'] = 'jpg|png|jpeg|pdf|doc|docx';
        $config['max_size'] = '10000';
        $config['max_width'] = '10000';
        $config['max_height'] = '10000';
        ...
        if ($mau_ke == "del") {
            $this->surat_model->delete_surat_klarifikasi_keluar($id_url);
            $this->session->set_flashdata('message', message_box('Data telah dihapus'));
            redirect('surat/surat_klarifikasi_keluar');
        } else if ($mau_ke == "cari") {
            if ($cari == "") {
                $data['data'] = $this->surat_model->cari_surat_klarifikasi_keluar_tgl($tglcari);
                $data['page'] = "surat/list_surat_klarifikasi_keluar";
            } else if ($tglcari == "") {
                $data['data'] = $this->surat_model->cari_surat_klarifikasi_keluar_key($cari);
                $data['page'] = "surat/list_surat_klarifikasi_keluar";
            } else {
                $data['data'] = $this->surat_model->cari_surat_klarifikasi_keluar_tgl_key($tglcari, $cari);
                $data['page'] = "surat/list_surat_klarifikasi_keluar";
            }
        } else if ($mau_ke == "add") {
            $data['page'] = "surat/form_surat_klarifikasi_keluar";
        } else if ($mau_ke == "edit") {
            $data['datpil'] = $this->surat_model->select_surat_klarifikasi_keluar_id($id_url);
            $data['page'] = "surat/form_surat_klarifikasi_keluar";
        } else if ($mau_ke == "act_add") {
            if ($this->upload->do_upload('file_surat')) {
                $up_data = $this->upload->data('file_name');
                $this->surat_model->insert_surat_klarifikasi_keluar_with_file($no_surat, $tgl_surat, $tujuan, $perihal, $up_data, $asisten);
            } else {
                $this->surat_model->insert_surat_klarifikasi_keluar($no_surat, $tgl_surat, $tujuan, $perihal, $asisten);
            }
            $this->session->set_flashdata('message', message_box('Data telah ditambah. ' . $this->upload->display_errors()));
            redirect('surat/surat_klarifikasi_keluar');
        } else if ($mau_ke == "act_edit") {
            if ($this->upload->do_upload('file_surat')) {
                $up_data = $this->upload->data('file_name');
                $this->surat_model->update_surat_klarifikasi_keluar_with_file($no_surat, $tgl_surat, $tujuan, $perihal, $up_data, $id_surat, $asisten);
            } else {
                $this->surat_model->update_surat_klarifikasi_keluar($no_surat, $tgl_surat, $tujuan, $perihal, $id_surat, $asisten);
            }
            $this->session->set_flashdata('message', message_box('Data telah diperbaharui. ' . $this->upload->display_errors()));
            redirect('surat/surat_klarifikasi_keluar');
        } else {
            $data['data'] = $this->surat_model->select_surat_klarifikasi_keluar_limit($awal, $akhir);
            $data['page'] = "surat/list_surat_klarifikasi_keluar";
        }
        $data['title'] = "Surat keluar";
        $this->load->view('sibers/header', $data);
    ...
    public function surat_klarifikasi_keluar2() {
        ...
        /* pagination */
        $total_row = $this->surat_model->get_total_row_surat_klarifikasi_keluar2();
        $per_page = 10;
        $awal = $this->uri->segment(4, 0);
        $akhir = $per_page;
        $data['pagi'] = _page($total_row, $per_page, 4, site_url('surat/surat_klarifikasi_keluar2/page'));
        ...
        //upload config 
        $config['upload_path'] = './assets/upload/surat_klarifikasi_keluar2';
        $config['allowed_types'] = 'jpg|png|jpeg|pdf|doc|docx';
        $config['max_size'] = '10000';
        $config['max_width'] = '10000';
        $config['max_height'] = '10000';
        ...
        if ($mau_ke == "del") {
            $this->surat_model->delete_surat_klarifikasi_keluar2($id_url);
            $this->session->set_flashdata('message', message_box('Data telah dihapus'));
            redirect('surat/surat_klarifikasi_keluar2');
        } else if ($mau_ke == "cari") {
            if ($cari == "") {
                $data['data'] = $this->surat_model->cari_surat_klarifikasi_keluar2_tgl($tglcari);
                $data['page'] = "surat/list_surat_klarifikasi_keluar2";
            } else if ($tglcari == "") {
                $data['data'] = $this->surat_model->cari_surat_klarifikasi_keluar2_key($cari);
                $data['page'] = "surat/list_surat_klarifikasi_keluar2";
            } else {
                $data['data'] = $this->surat_model->cari_surat_klarifikasi_keluar2_tgl_key($tglcari, $cari);
                $data['page'] = "surat/list_surat_klarifikasi_keluar2";
            }
        } else if ($mau_ke == "add") {
            $data['page'] = "surat/form_surat_klarifikasi_keluar2";
        } else if ($mau_ke == "edit") {
            $data['datpil'] = $this->surat_model->select_surat_klarifikasi_keluar2_id($id_url);
            $data['page'] = "surat/form_surat_klarifikasi_keluar2";
        } else if ($mau_ke == "act_add") {
            if ($this->upload->do_upload('file_surat')) {
                $up_data = $this->upload->data('file_name');
                $this->surat_model->insert_surat_klarifikasi_keluar2_with_file($no_surat, $tgl_surat, $tujuan, $perihal, $up_data, $asisten);
            } else {
                $this->surat_model->insert_surat_klarifikasi_keluar2($no_surat, $tgl_surat, $tujuan, $perihal, $asisten);
            }
            $this->session->set_flashdata('message', message_box('Data telah ditambah. ' . $this->upload->display_errors()));
            redirect('surat/surat_klarifikasi_keluar2');
        } else if ($mau_ke == "act_edit") {
            if ($this->upload->do_upload('file_surat')) {
                $up_data = $this->upload->data('file_name');
                $this->surat_model->update_surat_klarifikasi_keluar2_with_file($no_surat, $tgl_surat, $tujuan, $perihal, $up_data,$id_surat, $asisten);
            } else {
                $this->surat_model->update_surat_klarifikasi_keluar2($no_surat, $tgl_surat, $tujuan, $perihal,$id_surat, $asisten);
            }
            $this->session->set_flashdata('message', message_box('Data telah diperbaharui. ' . $this->upload->display_errors()));
            redirect('surat/surat_klarifikasi_keluar2');
        } else {
            $data['data'] = $this->surat_model->select_surat_klarifikasi_keluar2_limit($awal, $akhir);
            $data['page'] = "surat/list_surat_klarifikasi_keluar2";
        }
        $data['title'] = "Surat keluar";
        $this->load->view('sibers/header', $data);
    ...
    public function surat_klarifikasi_masuk() {
        ...
        /* pagination */
        $total_row = $this->surat_model->get_total_row_surat_klarifikasi_masuk();
        $per_page = 10;
        $awal = $this->uri->segment(4, 0);
        $akhir = $per_page;
        $data['pagi'] = _page($total_row, $per_page, 4, site_url('surat/surat_klarifikasi_masuk/page'));
        ...
        //upload config 
        $config['upload_path'] = './assets/upload/surat_klarifikasi_masuk';
        $config['allowed_types'] = 'jpg|png|jpeg|pdf|doc|docx';
        $config['max_size'] = '10000';
        $config['max_width'] = '10000';
        $config['max_height'] = '10000';
        ...
        if ($mau_ke == "del") {
            $this->surat_model->delete_surat_klarifikasi_masuk($id_url);
            $this->session->set_flashdata('message', message_box('Data telah dihapus'));
            redirect('surat/surat_klarifikasi_masuk');
        } else if ($mau_ke == "cari") {
            if ($cari == "") {
                $data['data'] = $this->surat_model->cari_surat_klarifikasi_masuk_tgl($tglcari);
                $data['page'] = "surat/list_surat_klarifikasi_masuk";
            } else if ($tglcari == "") {
                $data['data'] = $this->surat_model->cari_surat_klarifikasi_masuk_key($cari);
                $data['page'] = "surat/list_surat_klarifikasi_masuk";
            } else {
                $data['data'] = $this->surat_model->cari_surat_klarifikasi_masuk_tgl_key($tglcari, $cari);
                $data['page'] = "surat/list_surat_klarifikasi_masuk";
            }
        } else if ($mau_ke == "add") {
            $data['page'] = "surat/form_surat_klarifikasi_masuk";
        } else if ($mau_ke == "edit") {
            $data['datpil'] = $this->surat_model->select_surat_klarifikasi_masuk_id($id_url);
            $data['page'] = "surat/form_surat_klarifikasi_masuk";
        } else if ($mau_ke == "act_add") {
            if ($this->upload->do_upload('file_surat')) {
                $up_data = $this->upload->data('file_name');
            ...
            $data['data'] = $this->surat_model->select_surat_klarifikasi_masuk_limit($awal, $akhir);
            $data['page'] = "surat/list_surat_klarifikasi_masuk";
        }
        $data['title'] = "Surat keluar";
        $this->load->view('sibers/header', $data);
